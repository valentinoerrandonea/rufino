...
export default async function DecisionDetailPage({ params }: PageProps) {
  ...
  const entry = await readProjectEntry(id, "decision", entryId);
  if (!entry) notFound();
  ...
  return (
    <div style={{ padding: "48px 56px 80px", maxWidth: 800, margin: "0 auto" }}>
      ...
      >
        ...
        <h1
          className="serif"
          style={{ fontSize: 30, fontWeight: 400, lineHeight: 1.15, marginBottom: 10 }}
        >
          {entry.title}
        </h1>
        {entry.when && (
          <div style={{ fontSize: 12, color: "var(--ink-3)" }}>
            Actualizada {relTimeLong(entry.when)}
          </div>
        )}
        {entry.tags.length > 0 && (
